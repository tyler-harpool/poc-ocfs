...
use crate::models::{Adr, UpdateAdr};
...
pub async fn create_adr(
    ...
    match result {
        Ok(record) => {
            ...
            log_request(&headers, Some(id), "ADR created");

            let response_data = Adr {
                id: Some(id),
                civ: input.civ.clone(),
                fam: input.fam.clone(),
...
pub async fn update_adr(
    ...
    Extension(pool): Extension<PgPool>,
    Path(id): Path<i32>,
    Json(input): Json<UpdateAdr>,
) -> impl IntoResponse {
    info!("Updating ADR with ID: {}", id);
        ...
        .await
    {
        Ok(result) => {
            if result.rows_affected() == 0 {
                error!("No ADR found with ID: {}", id);
                (StatusCode::NOT_FOUND, "No ADR found").into_response()
            } else {
                info!("Successfully updated ADR with ID: {}", id);
                (StatusCode::OK, "ADR updated successfully").into_response()
            }
        }
        Err(e) => {
            error!(
...
pub async fn get_adr(
    ...
    match sqlx::query_as::<_, Adr>(query)
        ...
        .await
    {
        Ok(adr) => (StatusCode::OK, Json(adr)).into_response(),
        Err(e) => {
            error!("Failed to fetch ADR for id {}: {:?}", id, e);
            (StatusCode::NOT_FOUND, Json("ADR not found")).into_response()
        }
    }
}
...
pub async fn delete_adr(
    ...
    match sqlx::query(query).bind(id).execute(&pool).await {
        Ok(result) => {
            ...
            if affected == 0 {
                error!("ADR not found with ID: {}", id);
                (
                    StatusCode::NOT_FOUND,
                    Json(DeleteResponse {
                        message: format!("ADR not found with ID: {}", id),
                    }),
                )
                    .into_response()
            } else {
                info!(
                    "Successfully deleted {} ADR record(s) with ID: {}",
                    affected, id
                );
                (
                    StatusCode::NO_CONTENT,
                    Json(DeleteResponse {
                        message: format!("Successfully deleted ADR with ID: {}", id),
                    }),
                )
                    .into_response()
            }
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(DeleteResponse {
                    message: format!("Failed to delete ADR with ID: {}", id),
                }),
            )
                .into_response()
        }
...
pub async fn list_all_adrs(
    _headers: HeaderMap, // Prefix with an underscore if not used
    Extension(pool): Extension<PgPool>,
) -> impl IntoResponse {
    log_request(&_headers, None, "Listing all ADRs");
