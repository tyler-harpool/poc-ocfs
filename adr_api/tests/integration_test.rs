...
#[tokio::test]
async fn test_create_get_update_delete_adr() {
    dotenv().ok();
    ...
    // Step 1: Create ADR
    info!("Step 1: Creating ADR");
    let create_response = client
        .post(&format!("{}/adr", base_url))
    ...
    if create_status != 201 {
        panic!(
            "Failed to create ADR. Status: {}, Body: {}",
            create_status, create_body
        );
    }
    ...
    let adr_id = create_body
        .get("id")
        .and_then(|id| id.as_i64())
        .expect("ID missing") as i32;

    info!("Created ADR with ID: {}", adr_id);

    // Step 2: Get ADR
    info!("Step 2: Getting ADR");
    ...
    // Step 3: Update ADR
    info!("Step 3: Updating ADR with ID: {}", adr_id);
    let update_response = client
        .patch(&format!("{}/adr/{}", base_url, adr_id))
        .header("X-Test-Client", "IntegrationTest")
        .json(&json!({
    ...
    // Step 5: Delete ADR
    info!("Step 5: Deleting ADR with ID: {}", adr_id);
    let delete_response = client
        .delete(&format!("{}/adr/{}", base_url, adr_id))
        .header("X-Test-Client", "IntegrationTest")
        .send()
        .await
        .unwrap();
