...
#[tokio::test]
async fn test_create_get_update_delete_charge() {
    dotenv().ok();
    ...
    if create_status != 201 {
        panic!(
            "Failed to create charge. Status: {}, Body: {}",
            create_status, create_body
        );
    }
    ...
    let charge_id = create_body
        .get("id")
        .and_then(|id| id.as_i64())
        .expect("ID missing") as i32;

    info!("Created Charge with ID: {}", charge_id);

    // Step 2: Get Charge
    info!("Step 2: Getting Charge");
    ...
    // Step 3: Update Charge
    info!("Step 3: Updating Charge with ID: {}", charge_id);
    let update_response = client
        .patch(&format!("{}/charges/{}", base_url, charge_id))
        .header("X-Test-Client", "IntegrationTest")
        .json(&json!({
    ...
    // Step 5: Delete Charge
    info!("Step 5: Deleting Charge with ID: {}", charge_id);
    let delete_response = client
        .delete(&format!("{}/charges/{}", base_url, charge_id))
        .header("X-Test-Client", "IntegrationTest")
        .send()
        .await
        .unwrap();
    ...
    info!("Deleted Charge with ID: {}", charge_id);

    // Step 6: Verify Deletion
    info!(
        "Step 6: Verifying Deletion for Charge with ID: {}",
        charge_id
    );
    let get_response = client
        .get(&format!("{}/charges/{}", base_url, charge_id))
