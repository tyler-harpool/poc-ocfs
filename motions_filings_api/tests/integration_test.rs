...
use dotenv::dotenv;
use log::{debug, info};
use motions_filings_api::{create_app, setup_logging}; // Correctly reference the motions_filings_api crate
use reqwest::Client;
use serde_json::json;
...
#[tokio::test]
async fn test_create_get_update_delete_motion_filing() {
    dotenv().ok();
    ...
    // Step 1: Create Motion Filing
    info!("Step 1: Creating Motion Filing");
    let create_response = client
        .post(&format!("{}/motions_filings", base_url))
    ...
    if create_status != 201 {
        panic!(
            "Failed to create motion filing. Status: {}, Body: {}",
            create_status, create_body
        );
    }
    ...
    let motion_filing_id = create_body
        .get("id")
        .and_then(|id| id.as_i64())
        .expect("ID missing") as i32;

    info!("Created Motion Filing with ID: {}", motion_filing_id);
    ...
    info!("Updated Motion Filing with ID: {}", motion_filing_id);

    // Step 4: Verify Update
    info!(
        "Step 4: Verifying Update for Motion Filing with ID: {}",
        motion_filing_id
    );
    let get_response = client
        .get(&format!("{}/motions_filings/{}", base_url, motion_filing_id))
    ...
    info!("Deleted Motion Filing with ID: {}", motion_filing_id);

    // Step 6: Verify Deletion
    info!(
        "Step 6: Verifying Deletion for Motion Filing with ID: {}",
        motion_filing_id
    );
    let get_response = client
        .get(&format!("{}/motions_filings/{}", base_url, motion_filing_id))
