...
use dotenv::dotenv;
use log::{debug, info};
use orders_api::{create_app, setup_logging}; // Correctly reference the orders_api crate
use reqwest::Client;
use serde_json::json;
...
async fn test_create_get_update_delete_order() {
    ...
    if create_status != 201 {
        panic!(
            "Failed to create order. Status: {}, Body: {}",
            create_status, create_body
        );
    }
    ...
    let order_id = create_body
        .get("id")
        .and_then(|id| id.as_i64())
        .expect("ID missing") as i32;

    info!("Created Order with ID: {}", order_id);

    // Step 2: Get Order
    info!("Step 2: Getting Order");
    ...
    // Step 3: Update Order
    info!("Step 3: Updating Order with ID: {}", order_id);
    ...
    // Step 5: Delete Order
    info!("Step 5: Deleting Order with ID: {}", order_id);
