...
use dotenv::dotenv;
use log::{debug, info};
use post_trial_api::{create_app, setup_logging}; // Correctly reference the post_trial_api crate
use reqwest::Client;
use serde_json::json;
...
#[tokio::test]
async fn test_create_get_update_delete_post_trial() {
    dotenv().ok();
    ...
    // Step 1: Create Post Trial
    info!("Step 1: Creating Post Trial");
    let create_response = client
        .post(&format!("{}/postTrial", base_url))
    ...
    if create_status != 201 {
        panic!(
            "Failed to create post trial. Status: {}, Body: {}",
            create_status, create_body
        );
    }
    ...
    let post_trial_id = create_body
        .get("id")
        .and_then(|id| id.as_i64())
        .expect("ID missing") as i32;

    info!("Created Post Trial with ID: {}", post_trial_id);

    // Step 2: Get Post Trial
    info!("Step 2: Getting Post Trial");
    let get_response = client
        .get(&format!("{}/postTrial/{}", base_url, post_trial_id))
        .header("X-Test-Client", "IntegrationTest")
        .send()
        .await
        .unwrap();
    ...
    // Step 3: Update Post Trial
    info!("Step 3: Updating Post Trial with ID: {}", post_trial_id);
    let update_response = client
        .patch(&format!("{}/postTrial/{}", base_url, post_trial_id))
        .header("X-Test-Client", "IntegrationTest")
        .json(&json!({
    ...
    info!("Updated Post Trial with ID: {}", post_trial_id);

    // Step 4: Verify Update
    info!(
        "Step 4: Verifying Update for Post Trial with ID: {}",
        post_trial_id
    );
    let get_response = client
        .get(&format!("{}/postTrial/{}", base_url, post_trial_id))
    ...
    // Step 5: Delete Post Trial
    info!("Step 5: Deleting Post Trial with ID: {}", post_trial_id);
    let delete_response = client
        .delete(&format!("{}/postTrial/{}", base_url, post_trial_id))
        .header("X-Test-Client", "IntegrationTest")
        .send()
        .await
        .unwrap();
    ...
    info!("Deleted Post Trial with ID: {}", post_trial_id);

    // Step 6: Verify Deletion
    info!(
        "Step 6: Verifying Deletion for Post Trial with ID: {}",
        post_trial_id
    );
    let get_response = client
        .get(&format!("{}/postTrial/{}", base_url, post_trial_id))
