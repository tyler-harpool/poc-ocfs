...
use dotenv::dotenv;
use log::{debug, info};
use probate_api::{create_app, setup_logging}; // Correctly reference the probate_api crate
use reqwest::Client;
use serde_json::json;
...
#[tokio::test]
async fn test_create_get_update_delete_probate_review_monitor() {
    dotenv().ok();
    ...
    // Step 1: Create Probate Review Monitor
    info!("Step 1: Creating Probate Review Monitor");
    let create_response = client
        .post(&format!("{}/probateReviewMonitor", base_url))
    ...
    if create_status != 201 {
        panic!(
            "Failed to create probate review monitor. Status: {}, Body: {}",
            create_status, create_body
        );
    }
    ...
    let probate_review_monitor_id = create_body
        .get("id")
    ...
    info!(
        "Step 4: Verifying Update for Probate Review Monitor with ID: {}",
    ...
    info!(
        "Step 6: Verifying Deletion for Probate Review Monitor with ID: {}",
        probate_review_monitor_id
    );
    let get_response = client
        .get(&format!("{}/probateReviewMonitor/{}", base_url, probate_review_monitor_id))
