...
#[tokio::test]
async fn test_create_get_update_delete_sanction() {
    dotenv().ok();
    ...
    // Step 1: Create Sanction
    info!("Step 1: Creating Sanction");
    let create_response = client
        .post(&format!("{}/sanctions", base_url))
    ...
    if create_status != 201 {
        panic!(
            "Failed to create sanction. Status: {}, Body: {}",
            create_status, create_body
        );
    }
    ...
    let sanction_id = create_body
        .get("id")
        .and_then(|id| id.as_i64())
        .expect("ID missing") as i32;

    info!("Created Sanction with ID: {}", sanction_id);

    // Step 2: Get Sanction
    info!("Step 2: Getting Sanction");
    let get_response = client
        .get(&format!("{}/sanctions/{}", base_url, sanction_id))
        .header("X-Test-Client", "IntegrationTest")
        .send()
        .await
        .unwrap();
    ...
    // Step 3: Update Sanction
    info!("Step 3: Updating Sanction with ID: {}", sanction_id);
    let update_response = client
        .patch(&format!("{}/sanctions/{}", base_url, sanction_id))
        .header("X-Test-Client", "IntegrationTest")
        .json(&json!({
    ...
    info!("Updated Sanction with ID: {}", sanction_id);

    // Step 4: Verify Update
    info!(
        "Step 4: Verifying Update for Sanction with ID: {}",
        sanction_id
    );
    let get_response = client
        .get(&format!("{}/sanctions/{}", base_url, sanction_id))
    ...
    // Step 5: Delete Sanction
    info!("Step 5: Deleting Sanction with ID: {}", sanction_id);
    let delete_response = client
        .delete(&format!("{}/sanctions/{}", base_url, sanction_id))
        .header("X-Test-Client", "IntegrationTest")
        .send()
        .await
        .unwrap();
    ...
    info!("Deleted Sanction with ID: {}", sanction_id);

    // Step 6: Verify Deletion
    info!(
        "Step 6: Verifying Deletion for Sanction with ID: {}",
        sanction_id
    );
    let get_response = client
        .get(&format!("{}/sanctions/{}", base_url, sanction_id))
