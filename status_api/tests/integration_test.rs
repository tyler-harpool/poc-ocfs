...
#[tokio::test]
async fn test_create_get_update_delete_status() {
    dotenv().ok();
    ...
    info!("Step 1: Creating Status");
    let create_response = client
        .post(&format!("{}/status", base_url))
    ...
    if create_status != 201 {
        panic!(
            "Failed to create status. Status: {}, Body: {}",
            create_status, create_body
        );
    }
    ...
    let status_id = create_body
        .get("id")
        .and_then(|id| id.as_i64())
        .expect("ID missing") as i32;

    info!("Created Status with ID: {}", status_id);

    // Step 2: Get Status
    info!("Step 2: Getting Status");
    ...
    info!("Step 3: Updating Status with ID: {}", status_id);
    let update_response = client
        .patch(&format!("{}/status/{}", base_url, status_id))
        .header("X-Test-Client", "IntegrationTest")
        .json(&json!({
    ...
    info!("Step 5: Deleting Status with ID: {}", status_id);
    let delete_response = client
        .delete(&format!("{}/status/{}", base_url, status_id))
        .header("X-Test-Client", "IntegrationTest")
        .send()
        .await
        .unwrap();
    ...
    info!("Deleted Status with ID: {}", status_id);

    // Step 6: Verify Deletion
    info!(
        "Step 6: Verifying Deletion for Status with ID: {}",
        status_id
    );
    let get_response = client
        .get(&format!("{}/status/{}", base_url, status_id))
